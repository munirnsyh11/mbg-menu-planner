...
function CreateMenuModal({ open, onClose, onSaved }) {
  ...
  // Reference AKG values (sekolah dasar ~ 1800 kcal)
  const akgRef = { calories: 1800, protein: 50, fat: 60, carbohydrate: 260 };
  const akgPct = {
    calories:     Math.round((totals.calories     / akgRef.calories)     * 100),
    protein:      Math.round((totals.protein      / akgRef.protein)      * 100),
    fat:          Math.round((totals.fat          / akgRef.fat)          * 100),
    carbohydrate: Math.round((totals.carbohydrate / akgRef.carbohydrate) * 100),
  };
        ...
        <div className="lg:col-span-1">
          <div className="rounded-xl border border-slate-200 bg-slate-50/50 p-4 sticky top-0">
            ...
            <div className="space-y-3">
              <AkgBar label="Kalori" value={totals.calories.toFixed(0)} unit="kkal" pct={akgPct.calories} target={akgRef.calories} />
              <AkgBar label="Protein" value={totals.protein.toFixed(1)} unit="g" pct={akgPct.protein} target={akgRef.protein} />
              <AkgBar label="Lemak" value={totals.fat.toFixed(1)} unit="g" pct={akgPct.fat} target={akgRef.fat} />
              <AkgBar label="Karbo" value={totals.carbohydrate.toFixed(1)} unit="g" pct={akgPct.carbohydrate} target={akgRef.carbohydrate} />
            </div>
            <div className="mt-4 pt-3 border-t border-slate-200 text-[11px] text-slate-500">
              Referensi AKG anak SD ≈ 1800 kkal / 50g protein.
            </div>
          </div>
        </div>
      </div>
